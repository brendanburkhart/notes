...
\begin{thm}\label{modular-congruence}
...
\begin{proof}
    By Fermat's little theorem, since $n$ is prime and $n$ does not divide $x$ (since $x < n$), we have $x^{n-1} \equiv 1 \pmod n$. Therefore, $x \odot_n x^{n-2} \equiv 1 \pmod n$, so $x^{-1} = x^{n-2} \bmod n$.
\end{proof}

\section{Fields}

\begin{defn}
    A \emph{field} is a set $F$ equipped with two binary operations called addition and multiplication (denoted by $+$ and $\cdot$ respectively), however they do not necessarily correspond to addition and multiplication on the real numbers.

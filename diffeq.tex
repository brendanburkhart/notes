...
Given a solution $y_1(t)$ to $y''(t) + p(t)y' + q(t)y = 0$, we can assume that $y_2(t) = v(t)y_1(t)$ is another solution for some $v(t)$. It follows that \begin{align*}
    y_2' &= v'y_1 + vy_1' \\
    y_2'' &= v''y_1 + 2v'y_1' + vy_2''.
\end{align*}
Then, substituting $y_2(t)$ into the differential equation we obtain
\begin{align*}
...
\end{align*}
This is in fact a first order linear differential equation in $v'$, so the \emph{order} of this problem has been reduced from a second order to just a first order differential equation.
...
\begin{rmk}\proofbreak
    \begin{itemize}
        ...
        \item If the homogeneous term is contains a polynomial of degree $n$, then the guess should contain a polynomial with undetermined coefficients also of degree $n$.
    \end{itemize}
\end{rmk}

\begin{prop}\label{linear-nonhomogeneous-sum}
    If $Y_1(t)$ and $Y_2(t)$ are solutions to
    \begin{align*}
        ay'' + by' + cy &= g_1(t), \\
        ay'' + by' + cy &= g_2(t)
    \end{align*} respectively, then $Y_1(t) + Y_2(t)$ is a solution to the differential equation with nonhomogeneous term $g_1(t) + g_2(t)$.
\end{prop}

\begin{exmp}
    Let \[y'' - 3y' - 4y = 3e^{2t} + 2\sin(t) - 8e^{t}\cos(2t).\]
    We can separated this into
    \begin{align*}
        y'' - 3y' - 4y &= 3e^{2t} \\
        y'' - 3y' - 4y &= 2\sin(t) \\
        y'' - 3y' - 4y &= -8e^{t}\cos(2t).
    ...
    Then we have the solution $Y = Y_1 + Y_2 + Y_3$ by Proposition \ref{linear-nonhomogeneous-sum}.
\end{exmp}

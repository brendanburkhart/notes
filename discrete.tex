...
\begin{rmk}
    $\complementof{(A \triangle B)} = A \intersection B$ when $U = A \union B$.
\end{rmk}
...
\begin{thm}{De Morgan's Laws}\label{demorgan-set}\proofbreak
    \begin{enumerate}
        \item $\complementof{(A \intersection B)} = \complementof{A} \union \complementof{B}$
        \item $\complementof{(A \union B)} = \complementof{A} \intersection \complementof{B}$
    \end{enumerate}
\end{thm}
...
\begin{thm}{Cantor's Theorem}\label{cantors-theorem}\proofbreak
    Let $A$ be a set, and let $f: A \to 2^A$. Then $f$ cannot be surjective.
\end{thm}

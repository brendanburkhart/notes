...
\begin{proof}
    By Theorem \ref{commutator-normal-subgroup}, $[G,G]$ is normal in $G$. If $H$ is abelian, then for any $x, y \in G$, so $Hxy = Hyx$ since $G/H$ is abelian. Then $Hxyx^{-1}y^{-1} = H$, and so $[x, y] \in H$. Therefore, $[G, G] \subseteq H$.

    If $[G, G] \subseteq H$, then for any $xH, yH \in G/H$ we have $xyx^{-1}y^{-1}H = H$ since $xyx^{-1}y^{-1}$ is in $[G,G]$ and therefore in $H$. It follows that $xHyH = yHxH$.
\end{proof}

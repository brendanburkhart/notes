...
Given a linear program and corresponding basic tableau, we have $y = c_B^{\transpose}B^{-1}$ and $x = B^{-1}b$. Recall that $y$ is an optimal solution to the dual problem when:
\begin{itemize}
    \item $Ax = b$,
    \item $x \geq 0$,
...
\begin{exmp}
    Let $A \in M_{n\times n}(\R)$ be symmetric and positive definite, $b \in \R^n$ be non-zero, and $c \in \R$ be positive. Consider the problem of minimizing $f(x) = \frac{1}{2}x^{\transpose}Ax$ such that $g(x) = b^{\transpose}x + c \leq 0$. Imagine we want to find all KKT points and their multipliers. Note that
    \begin{align*}
        \nabla f(x) = Ax,\;\;
        \nabla g(x) = b.
    \end{align*}
    ...
    \begin{align*}
        Ax + \lambda b = \vec{0},
    \end{align*}
    for $\lambda \geq 0$. We then have $x = -\lambda A^{-1}b$. For $x$ to be feasible and $g$ be active, we need $b^{\transpose}\left(-\lambda A^{-1}b\right) + c = \vec{0}$ and so the only KKT multiplier and point is
    \begin{align*}
        \lambda = \frac{c}{b^{\transpose}A^{-1}b},\;\; x = -\frac{c}{b^{\transpose}A^{-1}b}A^{-1}b.
    \end{align*}
\end{exmp}

\begin{exmp}
    Consider the linear program of minimizing $c^{\transpose}x$ such that $Ax \geq b$ and $x \geq 0$. Alternatively, minimize $f(x) = c^{\transpose}x$ such that
    \begin{align*}
    ...
    \begin{align*}
        A^{\transpose}y + z &= c, \\
        y^{\transpose}Ax &= y^{\transpose}b = b^{\transpose}y, \\
        z^{\transpose}x &= 0.
    \end{align*}
    Therefore, $z = c - A^{\transpose}y$, and so $z^{\transpose}x = 0$ implies that $c^{\transpose}x = b^{\transpose}y$ and $z \geq \vec{0}$ implies that $A^{\transpose}y \leq c$. Altogether, the KKT conditions then require that
    \begin{align*}
        Ax &\geq b, \;\; x \geq \vec{0}, \\
        A^{\transpose}y &\leq c, \;\; y \geq \vec{0},
    \end{align*}
    and $c^{\transpose}x = b^{\transpose}y$. Notice that these are equivalent to the requirements for complementary slackness \ref{complementary-slackness}.
\end{exmp}

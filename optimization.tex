...
\begin{defn}
    ...
    find $x \in \R^n$ that maximizes or minimizes $C \cdot x$ (also commonly seen as $\transposeof{C}x$) such that $Ax = b$ and $x \vec{0}$. The problem is said to be in \emph{canonical form} if the constraints are instead in the form $Ax \geq b$ (or $Ax \leq b$).
\end{defn}
...
\begin{prop}
    Minimizing $\transposeof{C}x$ is equivalent to maximizing $-\transposeof{C}x$ (and so maximizing $\transposeof{C}$ is equivalent to minimizing $\transposeof{C}x$).
\end{prop}
...
Given a linear program and corresponding basic tableau, we have $y = \transposeof{c_B}B^{-1}$ and $x = B^{-1}b$. Recall that $y$ is an optimal solution to the dual problem when:
\begin{itemize}
    \item $Ax = b$,
    \item $x \geq 0$,
...
\begin{proof}\proofbreak
    ...
    ($\impliedby$) Assume that $\epigraph(f)$ is a convex set. Then for any $\transposeof{[x_1, y_1]}, \transposeof{[x_2, y_2]} \in \epigraph(f)$ and $\lambda \in [0, 1]$, we know that
    \begin{align*}

...
\begin{defn}
    A symmetric matrix $A \in M_{n\times n}(\R)$ is \emph{positiive definite} is $z^{\transpose}Mz$ is strictly positive for \emph{every} non-zero real $z \in \R^n$. Also Hermitian matrix $z^{*}Hz$.
\end{defn}

\begin{thm}
    Following are all equivalent:
    \begin{itemize}
        \item posiitive-definite,
        \item congruent with positive diagonal matrix,
        \item symmetric/Hermitian and eigenvalues are real and positive,
        \item all leading principal minors are positive,
        \item $M = B^{*}B$ where $B$ is invertible.
    \end{itemize}
\end{thm}

\begin{rmk}
    Positive-semi definite and invertible (so eigenvalues can't be zero) then implies positive definite.
\end{rmk}

...
\begin{defn}
    Given languages $A$ and $B$, the \emph{concatenation} of $A$ and $B$, denoted by $A + B$, is
    \begin{align*}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \end{dcases}$ and $\delta(q, \varepsilon) = \begin{dcases}
            \{q_B\}, &q \in F_A \\
        ...
        \item $F = F_B$.
    \end{itemize}
    Notice that if $w = \alpha\beta$, then the input $\alpha\varepsilon\beta$ will cause $M$ to end in an accept state, and so any such $w$ is accepted. If $w$ is accepted, then since the only transition possible from states in $Q_A$ to those in $Q_B$ (which contains all possible accept states) is an $\varepsilon$-transition from a state in $F_A$, and so $w = \alpha\beta$ for some $\alpha \in A$ and $\beta \in B$.
...
\begin{proof}
    Given a deterministic finite automata $M = (Q, \Sigma, \delta, q_0, F)$, we make a number of small modifications in order to produce a non-determinstic finite automata $N$ recognizing $A^{*}$ and thus proving it is regular. Specifically, we add $\varepsilon$-transitions from every accept state of $M$ back to $q_0$ --- this allows $N$ to recognize any string you can form by concatenating strings from $A$. We also introduce a new start state with a single $\varepsilon$-transition to the original start state, and make this new start state an accept state, thus allowing $N$ to also recognize $\varepsilon$.
    ...
    \begin{itemize}
        ...
        \item $\delta_N(q, a) = \begin{dcases}
            \delta(q, a), &q \in Q \\
            \{\}, &q = q_N \\
        \end{dcases}$ and $\delta_N(q, \varepsilon) = \begin{dcases}
            \delta(q, \varepsilon) \union \{q_0\}, &q \in F, q \neq q_N \\
